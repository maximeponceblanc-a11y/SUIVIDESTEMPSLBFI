...
# 💡 EXPLICATIONS : COMMENT MODIFIER VOTRE LIEN ONEDRIVE / SHAREPOINT À LA MAIN ?
# ─────────────────────────────────────────────────────────────────────────────
# 1. Allez sur votre OneDrive ou SharePoint, faites un clic droit sur le fichier -> "Partager".
# 2. Choisissez "Toute personne disposant du lien" (en décochant "Autoriser la modification"). Copiez le lien.
# 3. Modifiez ce lien manuellement selon l'un des deux cas suivants :
#
#    CAS A : Votre lien contient ":x:/g/" ou ":x:/s/"
#    -> Supprimez tout ce qu'il y a APRÈS le point d'interrogation (le ?e=...) et remplacez par : ?download=1
#    Exemple transformé : https://monentreprise.sharepoint.com/:x:/g/personal/.../Fichier.xlsm?download=1
#
#    CAS B : Votre lien se termine par "action=default"
#    -> Remplacez simplement "action=default" par "action=download"
#    Exemple transformé : https://monentreprise.sharepoint.com/.../doc.aspx?sourcedoc=...&action=download
#
# 4. UNE FOIS LE LIEN MODIFIÉ : Ne le collez PAS en clair dans le code ci-dessous.
#    Collez-le dans les "Secrets" de votre application sur le site de Streamlit (Paramètres -> Secrets) comme ceci :
#    EXCEL_DATA_URL = "VOTRE_LIEN_MODIFIE_ICI"
# ─────────────────────────────────────────────────────────────────────────────


# ── Chargement des données ────────────────────────────────────────────────────
# On ajoute un "ttl=3600" (1 heure) pour que Streamlit garde le fichier en mémoire cache
# et évite de retélécharger le fichier Excel depuis SharePoint à chaque fois qu'un utilisateur clique sur un filtre.
@st.cache_data(ttl=3600)
def load_data(path_or_url_or_bytes):
    df = pd.read_excel(path_or_url_or_bytes, sheet_name="ordres_fabrication", engine="openpyxl")
    ...
    # Normalisation propre des identifiants (supprime les ".0" générés par Excel)
    def clean_id(val):
...
# Récupération sécurisée du lien depuis les secrets de Streamlit Cloud
...
if excel_url:
    try:
        # Tente de charger les données directement depuis le lien SharePoint/OneDrive
        df_all = load_data(excel_url)
        src_label = "https://ponceblanc.sharepoint.com/:x:/g/IQBl1BgjArMlT7_a6jwg12HlAZbS703c_UykEd4AdrTisTs?download=1"
...
# Mode de secours : Si le lien cloud n'est pas configuré ou s'il a planté
if df_all is None:
...
# ── Ligne filtres (2 lignes équilibrées) ──────────────────────────────────────
row1 = st.columns([1.1, 1.1, 1, 1, 1, 0.8])
row2 = st.columns([1, 1, 1, 3.2])          

date_debut = row1[0].date_input("📅 Date début",
    min_value=data_min, max_value=data_max, key="date_debut")
date_fin   = row1[1].date_input("📅 Date fin",
    min_value=data_min, max_value=data_max, key="date_fin")
...
# Selectbox dossier de fabrication
...
k1.markdown(f'<div class="kpi-card"><div class="kpi-label">⏱ Temps de production total</div>'
            f'<div class="kpi-value">{total_op:,.2f} h</div></div>', unsafe_allow_html=True)
k2.markdown(f'<div class="kpi-card"><div class="kpi-label">📋 Temps devis total</div>'
            f'<div class="kpi-value">{total_devis:,.2f} h</div></div>', unsafe_allow_html=True)
k3.markdown(f'<div class="kpi-card"><div class="kpi-label">⚡ Efficacité (Prod / Devis)</div>'
            f'<div class="{eff_cls}">{efficacite:.2f}</div></div>', unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ── Graphique temporel maille dynamique ──────────────────────────────────────
maille     = st.radio("Maille temporelle", ["Semaine","Mois","Année"], index=1,
                      horizontal=True, key="maille")
...
fig_maille = go.Figure([
    go.Bar(x=by_maille[maille_col], y=by_maille["tps_op_h"],
           name="Temps de production", marker_color=C_PROD),
    go.Bar(x=by_maille[maille_col], y=by_maille["tps_devis_h"],
           name="Temps devis", marker_color=C_DEVIS),
])
...
# ── Graphique par poste ───────────────────────────────────────────────────────
by_poste = (df.groupby("poste")[["tps_op_h","tps_devis_h"]]
              .sum().reset_index().sort_values("tps_devis_h", ascending=True))
fig_poste = go.Figure([
    go.Bar(y=by_poste["poste"], x=by_poste["tps_op_h"],
           name="Temps de production", marker_color=C_PROD, orientation="h"),
    go.Bar(y=by_poste["poste"], x=by_poste["tps_devis_h"],
           name="Temps devis", marker_color=C_DEVIS, orientation="h"),
])
...
# ── Tableau par dossier — avec N° devis ──────────────────────────────────────
by_dossier = (df.groupby(["numero_dossier","numero_devis","client","reference"])
                [["tps_op_h","tps_devis_h"]].sum().reset_index())
by_dossier["ratio"] = (by_dossier["tps_op_h"] /
                       by_dossier["tps_devis_h"].replace(0, float("nan"))).round(2)
by_dossier = by_dossier.sort_values("tps_devis_h", ascending=False).reset_index(drop=True)
by_dossier.columns = ["N° dossier","N° devis","Client","Référence",
                      "Tps prod (h)","Tps devis (h)","Ratio"]
...
with col_tbl:
    if dossier_actif != "Tous" or devis_actif != "Tous":
        ...
        st.markdown(
            f'<div style="background:#e8f4fd;border-left:4px solid #1a3a6b;padding:6px 12px;'
            f'border-radius:4px;margin-bottom:6px;font-size:13px;">'
            f'🔍 Filtré sur : {" · ".join(label)}</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div style="background:#f0f2f6;padding:6px 12px;border-radius:4px;'
            'margin-bottom:6px;font-size:13px;color:#666;">'
            '👆 Cliquez sur une ligne pour filtrer tout le dashboard sur ce dossier</div>',
            unsafe_allow_html=True,
        )

    tbl_event = st.dataframe(
        by_dossier, use_container_width=True, height=270, hide_index=True,
        on_select="rerun", selection_mode="single-row", key="tbl_dossier",
    )

    selected_rows = tbl_event.selection.rows if tbl_event.selection else []
    had_sel = st.session_state.get("_last_tbl_sel", False)

    # --- CORRECTION 2 : Protection contre l'IndexError ---
    if selected_rows:
        row_index = selected_rows[0]
        
        # On vérifie que l'index existe toujours dans le tableau
    ...
    # Mise à jour de la mémoire seulement si la ligne est valide
...
by_op_tbl["ratio"] = (by_op_tbl["tps_op_h"] /
                      by_op_tbl["tps_devis_h"].replace(0, float("nan"))).round(2)
...
fig_op = go.Figure([
    go.Bar(x=by_op_chart["Opération"], y=by_op_chart["Tps prod (h)"],
           name="Temps de production", marker_color=C_PROD),
    go.Bar(x=by_op_chart["Opération"], y=by_op_chart["Tps devis (h)"],
           name="Temps devis", marker_color=C_DEVIS),
])
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown("---")
st.caption(f"📁 Source : `{src_label}` · {len(df_all):,} lignes totales · "
           f"{len(df):,} lignes filtrées · Période : {date_debut} → {date_fin}")